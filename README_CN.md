...
#### 版本历史
- **v0.0.2** (2025-12-27): 新增批量文件下载、公共文件下载功能，修复已知bug
...
#### 批量文件操作
- **批量文件上传**: 一次上传多个文件（最多10个）
- **批量文件下载**: 使用多个URL批量下载文件，URL之间用分号(;)分隔
- **公共文件下载**: 下载任何平台的公开文件，无需API密钥或授权
...
#### 4. 通过URL批量获取文件 (get_files_by_urls)

用于使用多个URL从华为云OBS批量检索文件的专用工具。
- **参数**:
  - `file_urls`: 华为云OBS中文件的多个URL，使用分号(;)分隔。一次最多下载10个文件。

#### 5. 通过URL获取公开文件 (get_public_file_by_url)

用于下载任何平台公开访问文件的专用工具。
- **参数**:
  - `file_url`: 任何平台可公开访问的文件URL（例如：华为云OBS、阿里云OSS、AWS S3等）
